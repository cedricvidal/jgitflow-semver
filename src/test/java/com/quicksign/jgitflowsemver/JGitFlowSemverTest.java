...
//        System.out.println("Git dir: " + gitDir);
    ...
    public void normal() throws Exception {
        ...
        assertEquals(Version.valueOf("0.0.0-1+sha." + sha()), infer());
        ...
        assertEquals(Version.valueOf("0.1.0-feature.first.3+sha." + sha()), infer());

        // Finish feature

        jGitFlow.featureFinish("first").call();

        assertEquals(Version.valueOf("0.1.0-dev.4+sha." + sha()), infer());

        // Start release

        jGitFlow.releaseStart("0.1.0").call();

        assertEquals(Version.valueOf("0.1.0-pre.4+sha." + sha()), infer());

        git.add().addFilepattern(appendToFile("README", "Prepare release\n").getName()).call();
        git.commit().setMessage("Prepare release").call();

        assertEquals(Version.valueOf("0.1.0-pre.5+sha." + sha()), infer());
        ...
        assertEquals(Version.valueOf("0.2.0-dev.1+sha." + sha()), infer());

        // Commit to develop while releasing
        git.add().addFilepattern(appendToFile("OTHER", "Some more\n").getName()).call();
        git.commit().setMessage("Some more").call();
        assertEquals(Version.valueOf("0.2.0-dev.2+sha." + sha()), infer());
        ...
        assertEquals(Version.valueOf("0.1.0"), infer());
        ...
        git.checkout().setName("develop").call();
        assertEquals(Version.valueOf("0.2.0-dev.3+sha." + sha()), infer());
        ...
        assertEquals(Version.valueOf("0.1.1-pre.1+sha." + sha()), infer());

        jGitFlow.hotfixFinish("0.1.1").call();

        git.checkout().setName("master").call();
        assertEquals(Version.valueOf("0.1.1"), infer());

        git.checkout().setName("develop").call();
        assertEquals(Version.valueOf("0.2.0-dev.4+sha." + sha()), infer());

        // Detached mode (often the case in CI servers)

        git.checkout().setName("develop").call();
        git.checkout().setName("HEAD").call(); // This checks out develop in detached mode
        assertEquals(Version.valueOf("0.1.1-detached.4+sha." + sha()), infer());
        ...
        assertEquals(Version.valueOf("0.1.1-fix.oups.1+sha." + sha()), infer());

        jGitFlow.hotfixFinish("oups").call();

        git.checkout().setName("master").call();
        assertEquals(Version.valueOf("0.1.1-2+sha." + sha()), infer());

        git.checkout().setName("develop").call();
        assertEquals(Version.valueOf("0.2.0-dev.7+sha." + sha()), infer());

    }

    private Version infer() throws Exception {
        return new SemVersionBuilder().build(jGitFlowSemver.infer(), conf);
    }
    ...
    private void assertPastCommit(ObjectId pastCommit, String pastBranch, Version expectedVersion) throws Exception {
        ...
        assertEquals(expectedVersion, infer());
